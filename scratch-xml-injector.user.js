    ...
    function blockToXML(id, indent) {
      ...
      for (const [name, inp] of Object.entries(b.inputs || {})) {
        ...
        if (Array.isArray(primary) && typeof primary[0] === 'number') {
          ...
          // Types 12=variable, 13=list are reporter blocks — must use <block>, not <shadow>
          if (primType === 12 || primType === 13) {
            out += `${pad}    <block type="${shadowType}">\n`;
            out += `${pad}      <field name="${fieldName}">${escXML(String(primVal))}</field>\n`;
            out += `${pad}    </block>\n`;
            out += `${pad}    <shadow type="math_number">\n`;
            out += `${pad}      <field name="NUM">0</field>\n`;
            out += `${pad}    </shadow>\n`;
          } else {
